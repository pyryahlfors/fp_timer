...
fp ={
	...
	createBuild: function() {
		...
		*/
		...
		if(fp.build.length === 0){
			console.log('FP build done:\r\n%c' + '"' + this.built.toString().replace(/,/g,'",\r\n"')+'"', 'background: #eee; color: #222');
			return false;
			}
		...
		if(eval(fp.build[0].confirmLoad) !== 'undefined') {
			fp.build.splice(0,1);
			}
		setTimeout(function(){fp.createBuild();}, 10);
		},
	...
	include: function ( inc ) {
		/*
			...
			path			: path, must include / at the end
			file			: filename
			init			: function to run after the script file is loaded
			disableCache	: true/false
			receipe			: true/false : returns true when the file is loaded
		*/
		...
		this.built.push(((typeof(inc.path) != 'undefined') ? inc.path : fp.path) + inc.file);
