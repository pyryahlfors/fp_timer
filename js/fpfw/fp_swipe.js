(function() {
	...
	var fp_module_swipe = (function() {
		return {
			...
			// Private function
				var mahData = function ( ) {return self.getBoundingClientRect();};
...
//						pages	: this.childNodes,
				...
				// 	bind window resize event listener
					if(params.scale){
						...
						window.addEventListener('resize', scaleElement, false);
						}

				// Private function; get translation coordinates
...
/* Do this better */
/* what the actual fuck is this anyways???? */
					...
					var initSwipeListener = function ( e ) { 
						...
						if(params.swipeStart){params.swipeStart({});};

						window.addEventListener(swipeEnd, endSwipeTrigger, false);
						document.addEventListener(swipeEnd, endSwipeTrigger, false);
						};
...
// Paginate
						paginate: function ( e ) {
							...
							// Call end swipe function if set
							if(params.endPaginate){
								params.endPaginate({
									current: container.currentPage,
									pages: container.pages.length
									});
								};
						...
						changePage: function(direction){
							container.width = (params.paginate) ? self.offsetWidth : self.scrollWidth;
							container.height = (params.paginate) ? self.offsetHeight : self.scrollHeight;
							if(container.currentPage+direction >= 0 && container.currentPage+direction <= container.pages.length-1) {
								container.currentPage = Math.abs(container.currentPage+direction);

								self.style[cssEngine+'Transition']= "all 300ms ease-out";
								setTranslateCoords(self, [ ((-1*container.currentPage)*container.width), 0 ] );
								for (var i =0, j = container.pages.length; i < j; i++) {
									container.pages[i].style.display = (i > container.currentPage-2 && i < container.currentPage+2) ? 'block' : 'none';
									};
